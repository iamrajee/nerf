echo "============================== START ============================="
echo "$(date|awk '{print $5}')" "parameter_reading" >> log.txt
...
echo "$(date|awk '{print $5}')" "update_and_install_conda_python" > log.txt
sudo apt update -y
...
echo "$(date|awk '{print $5}')" "clone_gsplat" >> log.txt
git clone https://github.com/graphdeco-inria/gaussian-splatting --recursive

echo "$(date|awk '{print $5}')" "install_plyfile" >> log.txt 
pip install -q plyfile

echo "$(date|awk '{print $5}')" "cd_gsplatting" >> log.txt 
cd gaussian-splatting

echo "$(date|awk '{print $5}')" "configure_conda" >> log.txt 
...
echo "$(date|awk '{print $5}')" "install_dgr_knn_whl" >> log.txt 
...
echo "$(date|awk '{print $5}')" "data_prep" >> log.txt
...
echo "$(date|awk '{print $5}')" "training" >> log.txt
...
echo "$(date|awk '{print $5}')" "render" >> log.txt
python3 render.py -m output/$output_folder

echo "$(date|awk '{print $5}')" "generate rendered video" >> log.txt
...
echo "$(date|awk '{print $5}')" "process complete you may download the result by runing scp in local host" >> log.txt
echo "============================== END ============================="
